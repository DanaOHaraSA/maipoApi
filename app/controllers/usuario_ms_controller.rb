class UsuarioMsController < ApplicationController
  ...
  def encuentra
    @usuario_m = UsuarioM.select("id").where(["email = :email and clave = :clave", {email: params[:email], clave: params[:clave]}])
  ...
  private
    ...
    def usuario_m_params
      params.require(:usuario_m).permit(:nombre, :email, :telefono, :clave, :calificacion, :rango , :imagenUrl)
    end
end
